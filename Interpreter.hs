...
toBasicBlocks body = let 
  ...
  -- By the properties of transform, bs' is a sequence of basic blocks, which start with a label and end with a goto or return
  tbs' = attach "start" (tbs ++ [([], gen Return)])  
  -- The first statement in tbs' cannot have empty label, thus only these cases in convert
...
-- | transform m st: transform st into a sequence of basic blocks, which start with a label and end with a goto or return;
